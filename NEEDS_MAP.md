# NEEDS_MAP.md

読者の悩み・状況別に育児グッズを整理する。

## Initial Needs

### 外出が大変

- 軽いベビーカー。
- 抱っこ紐。
- マザーズバッグ。
- 車なし育児向け用品。

### 狭い家で使いたい

- 折りたたみ用品。
- 省スペース収納。
- 多機能グッズ。

### ワンオペ育児

- 片手で使いやすい用品。
- 準備・片付けが楽な用品。
- 見守り用品。

### 保育園準備

- 名前付け用品。
- お昼寝用品。
- 通園バッグ。
- 食事用品。

### 旅行・帰省

- 持ち運びしやすい用品。
- 折りたたみグッズ。
- 移動中に使う用品。

## Published Article Mapping

既存記事を悩み別に整理し、内部導線を作るときの参照先にする。

### 外出が大変

- `articles/lightweight-stroller-train.html` — 車なし・電車移動向けの軽量ベビーカー確認軸。
- `articles/carrier-vs-stroller-first.html` — 抱っこ紐とベビーカーの優先順位。
- `articles/newborn-carrier-check.html` — 新生児期から使う抱っこ紐の確認点。
- `articles/childseat-taxi-check.html` — 車なし家庭の退院・タクシー利用時の確認点。

### 狭い家で使いたい

- `articles/small-home-birth-prep.html` — 狭い家・収納少なめの出産準備全体。
- `articles/stroller-entryway-storage.html` — 玄関が狭い家のベビーカー置き場。
- `articles/baby-bed-small-room-check.html` — 狭い家でのベビーベッド設置確認。
- `articles/baby-bath-storage-safety.html` — ベビーバスの置き場所・収納・安全確認。
- `articles/diaper-station-small-home.html` — おむつ替えとストック置き場の導線。
- `articles/bottle-feeding-setup-check.html` — 授乳・哺乳瓶まわりの省スペース準備。
- `articles/baby-clothes-minimal-prep.html` — ベビー服・肌着の買いすぎ防止。

### ワンオペ育児

- `articles/bottle-feeding-setup-check.html` — 夜間や片手対応を想定した授乳まわり。
- `articles/diaper-station-small-home.html` — 取りに行く回数を減らすおむつ替え配置。
- `articles/baby-gate-placement-check.html` — 家事動線上の立ち入り対策を考える入口。

### 保育園準備

- `articles/nursery-name-label-check.html` — 名前付け用品を買う前に、園指定・素材・収納場所を確認する入口。通園バッグ、食事用品は今後の候補。

### 旅行・帰省

- `articles/lightweight-stroller-train.html` — 電車・徒歩・バス中心の移動確認。
- `articles/newborn-carrier-check.html` — 移動時の抱っこ紐確認。
- `articles/childseat-taxi-check.html` — タクシーや家族の車を使う可能性の確認。

## Gap Ideas

- 保育園準備は名前付け用品の記事が公開済み。次は通園バッグ、食事用品、お昼寝用品のうち、園指定の確認が多い小トピックから追加する余地がある。
- ワンオペ育児は既存記事に要素が散っているため、将来的にまとめページか関連記事導線を作ると探しやすい。
- 旅行・帰省は外出記事と重なるため、価格・在庫・対象月齢を断定せず、持ち運びやすさと公式条件の再確認を中心に扱う。

## Notes

悩み別記事は不安を煽らず、選択肢と比較軸を整理する。
安全・医療・対象月齢・対象体重・リコール・取り付け条件は、記事化前にメーカー公式情報や公的情報で再確認する。
